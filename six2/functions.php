...
if (!isset($content_width))
{
	...
	add_image_size('3col',  420);
	add_image_size('7col',  1020);
	add_image_size('8col',  1170);
	add_image_size('12col', 1770);
	...
	function add_image_sizes($html, $id, $alt, $title, $align, $size) {
		...
		. '" data-src-four-retina="'  . $col_6[0]
		. '" data-src-six="'          . $col_6[0]
		. '" data-src-six-retina="'   . $col_12[0]
		. '" data-src-seven="'        . $col_7[0]
		. '" data-src-seven-retina="' . $col_12[0]
		. '" data-src-eight="'        . $col_8[0]
		. '" data-src-eight-retina="' . $col_12[0]
		. '"></div>';
...
/* ============================================================
   ...
   ============================================================ */
	...
	/* Custom function that returns either the category list or the roles list */
	
	function project_list($list){
		...
		$separator = ' &middot; ';
		$exclude = array('36', '37', '38', '39', '40', '41', '42');
		...
		if($categories){
			...
			foreach($categories as $category) {
				...
				if (!in_array($category->cat_ID, $exclude)){
					...
					$category_list .= '<a href="' . get_category_link( $category->cat_ID ) . '" title="' . esc_attr( sprintf( $category->name ) ) . '">' . $category->cat_name . '</a>' . $separator;
					
				} else {
					
					$role_count++;
					$role_list .= '<a href="' . get_category_link( $category->cat_ID ) . '" title="' . esc_attr( sprintf( $category->name )) . '">' . $category->cat_name . '</a>' . $separator;
			...
			if($list == 'categories'){
				...
				return '<span class="post-header_list">Categories:</span> ' . trim($category_list, $separator);
				
			} else{
				
				if($role_count > 1){
					
					return '<span class="post-header_list">Project Roles:</span> ' . trim($role_list, $separator);
					
				} else{
					
					return '<span class="post-header_list">Project Role:</span> ' . trim($role_list, $separator);
